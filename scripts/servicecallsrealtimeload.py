...
def loadData(inputFile, maxCount):
    ...
    with open(inputFile, 'r') as csvfile:
        ...
        for row in csvreader:
            ...
            # print(jsonData)
            index += 1
            conn.createServiceCallRealtime(jsonData)
            if index > maxCount:
                break


    #incidentReports = conn.getAll()
    #if(incidentReports is not None):
    #    if(len(incidentReports) > 2):
    #        incidentReport = conn.getIncidentReport(incidentReports[0])
    #        pp = pprint.PrettyPrinter(indent=4)
    #        pp.pprint(incidentReport)
            #conn.updateIncidentReport(incidentReports[0])
            #conn.deleteIncidentReport(incidentReports[1])
    #file.close()
